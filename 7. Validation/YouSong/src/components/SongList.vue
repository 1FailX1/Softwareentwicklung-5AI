...
export default {
  setup() {
    ...
    const fetchSongs = async (page = 0) => {
      try {
        console.log(page)
        console.log(`http://localhost:8080/api/songs/paginated?page=${page}`)
    ...
    const searchSongs = async (page = 0) => {
      try {
        //console.log(page)
        //console.log(`http://localhost:8080/api/songs/search/paginated?searchTerm=${searchTerm.value}&page=${page}&size=5`)
        const response = await axios.get(`http://localhost:8080/api/songs/search/paginated`, {
          params: {
